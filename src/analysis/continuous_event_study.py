"""
Continuous-score event study — mirrors dictionary_event_study.py but
sources the stance signal from continuous_scorer.py (LLM-assigned
-10..+10 intensity) instead of keyword counts. Same 60-day rolling
baseline, same downstream alignment/Granger/placebo machinery, so
results are directly comparable to both the discrete-LLM and
dictionary branches.

Usage:
    python -m src.analysis.continuous_event_study
"""
...
def build_events_continuous(scored_csv: str, baseline_window_days: int = 60) -> pd.DataFrame:
    ...
    logger.info("Continuous-score events after dedup: %d (from %d articles)", len(agg), len(df))
    return agg
