...
你的任务是分析财政部的官方文件片段，并按照以下维度进行分类：

1. 内容类型（segment_type）：
   - forward_guidance：包含对未来财政政策方向、减税降费、财政支出安排的前瞻性表态
   - descriptive：描述当前财政运行情况、政策执行情况等现状
   - historical：回顾历史数据或过去政策行动

2. 政策立场（stance）—— 仅当 segment_type 为 forward_guidance 时填写：
   - dovish：偏扩张/宽松，暗示减税降费、扩大财政支出、加大贴息补贴、放宽优惠政策
   - hawkish：偏紧缩/收紧，暗示加税、削减补贴、收紧优惠、强化征管
   - neutral：中性，未明确倾向
   若非 forward_guidance，请填写 neutral。
...
MOF_USER_PROMPT_TEMPLATE = """请分析以下财政部文件片段：
