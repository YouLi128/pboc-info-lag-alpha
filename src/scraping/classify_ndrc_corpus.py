"""
Classify NDRC listings corpus with LLM (broad-market extension branch).

Supports resume: skips URLs already successfully classified in the output CSV.

Usage:
    python -m src.scraping.classify_ndrc_corpus
"""
...
from src.scraping.ndrc_scraper import fetch_article_text
...
LISTINGS = Path("data/processed/ndrc_listings.csv")
...
def run(out: Path, listings_path: Path = LISTINGS) -> None:
    ...
    clf = StanceClassifier(source="ndrc")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=Path,
                        default=Path("data/processed/ndrc_classified.csv"))
